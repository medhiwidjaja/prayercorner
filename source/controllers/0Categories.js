enyo.kind({
    ...
    all: function(handleSuccess, handleError) {
        var sql = 'SELECT g.rowID, g.title, count(p.rowID) as count FROM groups g LEFT OUTER JOIN prayers p ON p.category = g.rowID GROUP BY g.rowID';
    ...
    createNew: function(model, handleSuccess, handleError) {
        var data = [{
            table: "groups",
            data: model
        }];
        // Run the query
        this.$.db.insertData(data, { "onSuccess": handleSuccess, "onError": handleError });
    },

    delete: function(model, handleSuccess, handleError) {
        var dbQuery = this.$.db.getDelete("groups", {rowID: model.rowID});
        this.$.db.query(dbQuery, { "onSuccess": handleSuccess, "onError": handleError });
    },
