enyo.kind({
	...
	components: [
		...
		{ kind: "enyo.Scroller", 
			...
			components: [
				...
				{ kind: "swash", type: "s", shade: "light" },
				{ content: "Active", classes: "basement-header" },
	...
	setup: function() {
		if (window.localStorage["PrayerCorner.setup"] !== "1") {
			...
			p.commit();
			var j = new PrayerList.JournalEntry({content:"New job", entryDate: "2 Feb 2013", prayerId: p.id});
			j.commit();
			j = new PrayerList.JournalEntry({content:"School registration accepted", entryDate: "15 Apr 2013", prayerId: p.id});
			j.commit();
			var b = new PrayerList.BibleVerse({text:"For you are my rock and my fortress; and for your name's sake you lead me and guide me;", passage:"Psalms 31:3", prayerId: p.id, version: "ESV"})
			b.commit();
			b = new PrayerList.BibleVerse({text:"And I will lead the blind in a way that they do not know, in paths that they have not known I will guide them. I will turn the darkness before them into light, the rough places into level ground. These are the things I do, and I do not forsake them.", passage:"Isaiah 42:16", prayerId: p.id, version: "ESV"})
			b.commit();
			...
			p.commit();
			j = new PrayerList.JournalEntry({content:"Paper proposal accepted", entryDate: "20 May 2013", prayerId: p.id});
			j.commit();
			j = new PrayerList.JournalEntry({content:"Fundings got approved", entryDate: "31 May 2013", prayerId: p.id});
			j.commit();
			b = new PrayerList.BibleVerse({text:"Now faith is the assurance of things hoped for, the conviction of things not seen.", passage:"Hebrew 11:1", prayerId: p.id, version: "ESV"})
			b.commit();
...
enyo.kind({
	...
	components: [
		...
		{ name: "cal", kind: "PrayerList.CalDate", calendarDate: new Date(), additionalStyles: "background-color: rgba(255,239,205,0.9); color: #7d0000;" },
